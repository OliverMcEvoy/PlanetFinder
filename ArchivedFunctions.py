...
def fetch_kepler_data_and_stellar_info_normalise_entire_curve(
    target, filter_type="savgol", randomise=False
):
    search_result = search_lightcurve(target, mission="Kepler")
    lc_collection = search_result.download_all()
    ...
    elif filter_type == "medfilt":def plot_light_curve(time, flux, flux_error=None):
    ...
    if len(lc_collection) > 0:
        star_data = lc_collection[0].meta
        stellar_params = {
            "stellar_radius": star_data.get(
                "RADIUS", np.nan
            ),  # Stellar radius in solar radii
            "temperature": star_data.get(
                "TEFF", np.nan
