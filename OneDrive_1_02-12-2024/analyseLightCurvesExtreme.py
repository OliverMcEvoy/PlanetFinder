...
def plot_light_curves(results):
    ...
    for i, (true_periods, predicted_periods, time_array, flux_with_noise, combined_light_curve, true_num_planets, predicted_num_planets) in enumerate(results):
      ...
      #  axs[i].plot(time_array, combined_light_curve, label='No-noise Light Curve', color='blue')
        ...
        axs[i].text(0.02, 0.98, actual_text, transform=axs[i].transAxes, fontsize=10, verticalalignment='top')
        axs[i].text(0.02, 0.92, predicted_text, transform=axs[i].transAxes, fontsize=10, verticalalignment='top')
...
def main():
    ...
    model = load_model("TheGoodModeltransit_model_5_percent_weight_decay.pth", device)
