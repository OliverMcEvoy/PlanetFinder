...
def generate_random_planet_systems(num_systems, max_planets_per_system, total_time):
    ...
    for _ in range(num_systems):
        ...
        star_radius = np.random.uniform(0.5, 2.0)
        observation_noise = np.random.uniform(0.001, 0.01)
        total_time = total_time

        for _ in range(num_planets):
            period = np.random.uniform(1, total_time / 2)
            planet_radius = np.random.uniform(0.01, 0.3)
            semi_major_axis = (period**2)**(1/3)
            eccentricity = 0  # Ideal eccentricity
            inclination = np.pi / 2
...
def main():
    ...
    with h5py.File(args.output_file, 'w') as hdf5_file:
        for iteration in tqdm(range(args.num_iterations), desc="Generating planet systems"):
            ...
            random_systems = generate_random_planet_systems(args.num_systems, max_planets_per_system, args.total_time)
